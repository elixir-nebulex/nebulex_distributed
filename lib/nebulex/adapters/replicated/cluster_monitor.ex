defmodule Nebulex.Adapters.Replicated.ClusterMonitor do
  ...
  def handle_continue(:bootstrap, %{adapter_meta: adapter_meta} = state) do
    # Get the list of nodes in the PG group
    cluster_nodes = Cluster.pg_nodes(adapter_meta.pg_group)

    # Bootstrap from the peers
    cluster_nodes
    |> List.delete(node())
    |> bootstrap_from_peers(adapter_meta)

    # After bootstrap, reset the GC interval on all nodes to synchronize
  ...
  defp maybe_reset_gc_interval(%{cache: cache} = adapter_meta, cluster_nodes) do
    ...
    if primary.__adapter__() == Nebulex.Adapters.Local do
      RPC.multicall(
        cluster_nodes,
